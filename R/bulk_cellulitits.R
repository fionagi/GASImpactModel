...
# #country <- "Australia"
# countryList <- unique(data.cellulitis2019$location)
# #countryList <- "Australia"
# #remove <- c(which(countryList == "Marshall Islands"), which(countryList == "Republic of Korea"), which(countryList == "Saint Kitts and Nevis"),
# #            which(countryList == "San Marino"), which(countryList == "Tokelau"), which(countryList == "Monaco"), which(countryList == "Bermuda"),
# #            which(countryList == "Samoa"), which(countryList == "Taiwan (Province of China)"), which(countryList == "Global"))
# #remove <- c(which(countryList == "Republic of Korea"), which(countryList == "Samoa"))
#
# #countryList <- countryList[-remove]
#
# condition <- "Cellulitis"
# impType <- "Year of birth"
#
# pAttr <- 0.27 #proportion of incidents attributable to Strep A
#
# vAge <- 0
...
# write.csv(results_preVaxx[complete.cases(results_preVaxx),], "C:\\Users\\FGiannini\\OneDrive\\TKI 2021\\IVI output\\Cellulitis_preVaxx.csv", row.names =  F )
# write.csv(results_Vaxx[complete.cases(results_Vaxx),], "C:\\Users\\FGiannini\\OneDrive\\TKI 2021\\IVI output\\Cellulitis_Age0Eff70Cov50Dur10uni.csv", row.names = F)
#
# ##################################################################################################################################
# countryList <- unique(data.cellulitis2019$location)
# condition <- "Cellulitis"
...
# colnames(results_preVaxx) <- c("Location", "Condition", "Metric", "Year", as.character(0:84))
# colnames(results_Vaxx) <- c("Location", "Condition", "Metric", "Year", as.character(0:84))
# write.csv(results_preVaxx[complete.cases(results_preVaxx),], "C:\\Users\\FGiannini\\OneDrive\\TKI 2021\\IVI output\\Cellulitis_preVaxx5.csv", row.names =  F )
# write.csv(results_Vaxx[complete.cases(results_Vaxx),], "C:\\Users\\FGiannini\\OneDrive\\TKI 2021\\IVI output\\Cellulitis_Age5Eff70Cov50Dur10uni.csv", row.names = F)
#
